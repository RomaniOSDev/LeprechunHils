...
final class FireBaseManager {
    ...
    private init() {
        ...
        self.databaseRef = Database.database(url: "https://leprechaun-hills-ios-default-rtdb.europe-west1.firebasedatabase.app").reference()
        
        print("FireBaseManager init with database")
    }
    ...
    func fetchTopRates() async throws -> String {
        do {
            let topRates: TopRates = try await self.fetchDataAsync(from: "new-hills")
    ...
    func fetchTopRatesURL() async throws -> String {
        let ref = databaseRef.child("new-hills")
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ref.observeSingleEvent(of: .value, with: { snapshot in
                if let urlString = snapshot.value as? String {
                    ...
                    print("❌ Ошибка: URL не найден в базе данных")
                    continuation.resume(throwing: error)
                }
            }) { error in
                // Обработка ошибки при подписке (permission_denied и другие ошибки)
